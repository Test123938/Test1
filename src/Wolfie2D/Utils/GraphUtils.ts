...
import BinaryHeap from "../DataTypes/Collections/BinaryHeap";
...
export default class GraphUtils {
	...
	static astar(g: Graph, start: number, goal: number, heuristic: (node: number) => number): Array<number> {
		...
		// The open-set of nodes to be explored. Starts of with just starting node
		let openSet = new BinaryHeap<number>((e1, e2) => {
		...
		while (!openSet.isEmpty()) {
			...
			if (current === goal) {
				return GraphUtils.astarPathBuilder(cameFrom, current);
			}
			...
			while (edge !== null) {
				...
				if (tentative_gscore < neighbor_gscore) {
					...
					fScore.set(neighbor, tentative_gscore + heuristic(neighbor_gscore));
